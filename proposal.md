...
Tarik Skubal is a pitcher for the Los Angeles Dodgers who entered the 2026 season coming off back-to-back American League Cy Young Award winning seasons (awarded to the best pitcher in the league) and is a consensus top ten player in the sport. After the conclusion of the 2026 World Series, he will be a free agent and can sign with whichever team gives him the highest offer.
...
## Literature Review
Predicting MLB contracts is something that has been done numerous ways by different people. There are many common ways of predicting a player's contract with the most popular being based on a player's future projected WAR output. WAR (Wins Above Replacement) is a common measure used to quantify how many wins a player is bringing to a team relative to a replacement level player. All-stars tend to have 5-6 WAR by the end of the season while MVP candidates are usually above 8 with all time seasons being around 10 WAR. This measure looks at a variety of aspects of the game such as a player's production on the plate as well as their effectiveness in the field. FanGraphs currently has the market set at $8 million per 1 WAR, so if a player finished the season with 5 WAR, they would be worth $40 million for that season (Clemens). This idea is taken and applied to a player's future projected WAR, so if a player is projected to have 25 WAR over the next 7 years, then a player would be projected to receive a 7 year/$200 million contract with this methodology. This was common practice for many years, and as of recently this idea has evolved, as $/WAR is no longer standard across all players. As has been observed with the previous free agent class, superstar players went for a higher $/WAR amount compared to replacement or above-average players: from 2020-2023, free agents who were projected over 2 WAR received over 40% more $/WAR than players who were projected between 0 and 1 WAR (Clemens). This non-linearity in $/WAR value has been something that many older models have not implemented, which can make a great addition to the new models we intend to build.

Going along the lines of projecting based on WAR, aging curves are another common component of contract projections. These look at the fluctuations in player performance as they age (Slowinski). Many aging curves typically have a concave-down shape, where a player sees an increase in performance from their early 20s to their peak, which is their late 20s to early 30s, and then a decline in performance as they age into their 30s. These are commonly used in WAR projections to see how a player's WAR will increase or decrease as they age. Similar to how $/WAR changes across time, aging curves can also change across time, as players from the 1990s have different aging curves than the players who play today (Clemens). This is why constantly updating the aging curve to see what new patterns or trends emerge can be very important, as older curves tend to be less reliable estimates (Slowinski).

Our main stakeholders for this project would be the upper management of the front office (GM, President of Baseball Ops) as well as the ownership group. A secondary stakeholder will be Skubal's agent, Scott Boras, as he also needs to be convinced that what we value him at is accurate and not underselling him. Due to both of these stakeholders, it is not only important that we can predict what Skubal should get paid, but we should also be able to explain why. This is a very important part of the negotiation process, as being able to explain what influences these projections is what will help the GM and ownership group make an informed decision, as well as prevent Skubal's agent from seeking a higher monetary value for no justifiable reason.
...
Given that Tarik Skubal is a pitcher, we would focus on metrics used to evaluate pitchers, such as spin rate and velocity, as opposed to hitter metrics like wRC+ or stats of that nature. In addition to that, we will be focusing on pitchers, as predicting the market for a pitcher is entirely different from the market for a hitter. These sources are very reliable, as they are frequently used by every MLB team as well as numerous research projects. They serve as the standard websites that store this data.

### Methods
Prior to building our models, we will use Principal Component Analysis (PCA) on our selected features to help reduce the dimensionality of our data, which will also help reduce any multicollinearity we could encounter in our model. This is especially important for our models, as our goal is not only to project what Skubal will make, but also to identify what is driving his projection.

After that, we will run three models: linear regression, random forest, and XGBoost. We will use multiple evaluation metrics such as RMSE and MAE to determine which model performed the best. After we select this model, we will use it to predict Skubal's contract.
...
| 11/20 - 12/1 | Build out presentation | Submit final project and prepare for presentation on 12/8 |

## Risks
While we don't have to worry about the risks posed in many projects such as data availability or integrity, there are still many pitfalls that our project could face. While we have a lot of data available to us, pitchers of Tarik Skubal's caliber are rare, as he is one of the top arms in the MLB. As a result of this, the models may have a difficult time predicting his value due to him being a relative outlier compared to the average MLB pitcher. While the MLB has a few ace-caliber pitchers at any given time, there are always changes in the MLB landscape that could affect how much a pitcher receives at a given point in time. Similarly, the MLB is proposing a salary cap, so even if our projection is highly accurate, a change in the financial structure of the league could drastically change the contract he receives. In terms of mitigation for these risks, some issues can be handled much better than others.

Some problems, such as the salary cap implications, will be tough to handle because it is unknown whether there will even be a salary cap implemented, and if so, what it will be and what other financial rules will accompany it. Unfortunately, due to this uncertainty, we will have to assume that the MLB financial structure will remain as it currently is, with no significant changes. On the other hand, issues such as the rarity of a player of Skubal's caliber are more within our control. This will require us to constantly work with the data to see how far back we should go, what pitchers we should include, and what variables are the best predictors of future pitcher performance. Constantly adjusting these factors will help us produce the most accurate models.
